...
var mobile_no;
	...
	function starter(mobile_no){
		...
		var code_no=getUrlParameter('num');
		...
		if(code_no){
			...
			$(window).on("load",function(){
				...
				$.ajax({
					...
					data:{
						flag:'thru_pc_check',
						mobile_no_check:code_no
						},
					success:function(data){
						if(data != 'false'){
							var info=JSON.parse(data);
							if(info[0]['status'] != 'authenticated'){
								window.location.replace("error.html");
								}
							else{
			...
			$(document).on("click",".add_rack",function(){
				var append="<div id='add_rack_o' class='file_exp_outer'><div class='file_exp_inner'>";
					...
					append+="<div class='file_exp_main'><h6>yout can add a rack and share with your team.</h6>";
					append+="<input type='text' class='add_rack_mem' name='add_rack_mem' placeholder='description' ><br/>";
...
if(isset($_GET['num'])){
	...
	$session_string=$_SESSION['string'];
	if($_SESSION['mobile_no']){
	
					$stm1=$mysqli->prepare("select `code_no`,`session_hash` from `drive`.`qrcodes` where `code_no`='".$mobile_no1."'");
					if($stm1->execute() && $stm1->num_rows>0){
					
						$stm1->bind_result($code_no,$session_hash);
						$stm1->store_result();
						$stm1->fetch();
						$user_browser=$_SESSION['HTTP_USER_AGENT'];
						$string=hash('sha512',$code_no.$user_browser);
						if($string===$session_string && $session_hash===$session_string){
							...
							if(isset($_POST['upload_submit'])){
								...
								if(file_exists($_FILES['file']['tmp_name']) || is_uploaded_file($_FILES['file']['tmp_name'])) {
									...
									$stm->execute();
									$stm->bind_result($mem_id,$name,$designation,$teams,$files,$shared_files,$number,$active_status);
									$stm->store_result();
									$stm->fetch();
